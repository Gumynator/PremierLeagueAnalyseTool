...
object main extends App{
   ...
   */
    ...
    val league = League(players)
    println("\nWelcome to the Premier League Analyse Tool")

    println("\nSum main statistics: ")
    val resultSum = league.sumStats();
    println((resultSum));

    println("\nAverage age for a team: ")
    var avg = 0.0
    var team = "Arsenal";
    league.avgAgeClub(team) match {
        case Some(value) => 
    ...
    val newClub = "Arsenal"
    val nation = "ENG"
    val transList = league.changeClub(newClub, playerTransfer)
    ...
    val playerAdd = league.addPlayer(newPlayer)
    println(playerAdd.getClub("Bobo FC", None))

    println("\nDelete player: ")
    println("\nCurrent Chelsea squad")
    println(league.getClub("Chelsea", Some("ENG")))
    val deltePlay = league.deletePlayer("Mason Mount", None)
    println("\nNew Chelsea squad")
    println(deltePlay.getClub("Chelsea", Some("ENG")))
    val deleteClub = league.deletePlayer(null, Some("Chelsea"))
    println("\nFinal Chelsea squad")
    ...
    val attribute = "goals"
    try {
        val sortGoals = league.sortPlayersBy(attribute,players)
    } catch {
    ...
    val attribute2 = "name"
    try {
    val sortName = league.sortPlayersBy(attribute2,players)
    } catch {
        case e: IllegalArgumentException =>
        println("Error: " + e.getMessage)
    }   
    ...
    val infoPlayer = league.getClub("Chelsea", None).take(1)
    println(infoPlayer)
    val preGoal = league.playerScored(scorer)
    ...
    val infoPlayer1 = league.getClub("Chelsea", None).take(1)
    println(infoPlayer1)
    val prePasses = league.passesStat(passer, 300, 450)
